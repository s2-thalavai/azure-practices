...
const KEYCLOAK_URL = process.env.KEYCLOAK_URL; // e.g., https://keycloak.example.com
...
app.http('authCallback', {
  ...
  handler: async (req, res) => {
    ...
    try {
      ...
      // Validate access token via JWKS
      ...
      jwt.verify(access_token, getKey, { algorithms: ['RS256'] }, (err, decoded) => {
        if (err) {
          res.status(401).send({ error: 'Invalid token', details: err.message });
        } else {
          res.status(200).send({ message: 'Token valid', user: decoded });
        }
      });
    } catch (err) {
      res.status(500).send({ error: 'Token exchange failed', details: err.message });
    }
  },
});
